...
static irqreturn_t mwifiex_wake_irq_wifi(int irq, void *priv)
{
	...
	if (cfg->irq_wifi >= 0) {
		dev_dbg(cfg->dev, "%s: wake by wifi", __func__);
...
/* This function parses device tree node using devicetree API.
 * If the device tree node exist and include interrupts attributes, this
 * function will also request platform specific wakeup interrupt.
 * @dev		: Device pointer
 * @match_table	: Device ID table to match
 * @wake_cfg	: Pointer in which a newly allocated mwifiex_plt_wake_cfg
		     ...
		     struct mwifiex_plt_wake_cfg **wake_cfg)
{
	...
	if (!dev->of_node || !of_match_node(matches, dev->of_node)) {
		dev_warn(dev, "device tree node not available\n");
		return -1;
	}
	...
	cfg->dev = dev;
	cfg->irq_wifi = irq_of_parse_and_map(dev->of_node, 0);
	if (!cfg->irq_wifi) {
		dev_err(dev, "fail to parse irq_wifi from device tree\n");
		return -1;
	}

	ret = devm_request_irq(dev, cfg->irq_wifi, mwifiex_wake_irq_wifi,
			       IRQF_TRIGGER_LOW, "wifi_wake", cfg);
	if (ret) {
		dev_err(dev, "Failed to request irq_wifi %d (%d)\n",
			cfg->irq_wifi, ret);
		return -1;
	}
...
EXPORT_SYMBOL_GPL(mwifiex_probe_of);
...
void mwifiex_enable_wake(struct mwifiex_plt_wake_cfg *wake_cfg)
{
	/* Enable platform specific wakeup interrupt */
	if (wake_cfg && wake_cfg->irq_wifi >= 0) {
		wake_cfg->wake_by_wifi = false;
		enable_irq(wake_cfg->irq_wifi);
		enable_irq_wake(wake_cfg->irq_wifi);
	}
}
EXPORT_SYMBOL_GPL(mwifiex_enable_wake);
